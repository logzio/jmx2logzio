...
public class Jmx2LogzioConfigurationTest {
    ...
    private static final String IN_MEMORY_TEST_ARGUMENTS = "LOGZIO_TOKEN=LogzioToken,SERVICE_NAME=CustomServiceName,SERVICE_HOST=CustomServiceHost,FROM_DISK=false,LISTENER_URL=http://listener.com:2222," +
            ...
            "DISK_SPACE_CHECKS_INTERVAL=13,FILE_SYSTEM_SPACE_LIMIT=80,CLEAN_SENT_METRICS_INTERVAL=14";
    ...
    private void clean() {
        try {
            FileUtils.deleteDirectory(new File(getShaQueueDir()));
        } catch (IOException e) {
            logger.error("couldn't remove temp metrics directory {}: {}", getShaQueueDir(), e.getMessage(), e);
        }
    }
    ...
    public void fromDiskConfigurationArgumentsParsingTest() {
        ...
        Assert.assertEquals(parent + senderParams.getQueueDir().getName(), getShaQueueDir());
